...
@section('title', __('reports.monthly_financial_report'))
...
<div class="report-hero">
    <div class="row align-items-center">
        <div class="col-lg-8">
            <h2><i class="fa fa-calendar"></i> {{ __('reports.monthly_financial_report') }}</h2>
    ...
    <div class="col-lg-6 mb-4">
        <div class="tile">
            ...
            <table class="table table-sm mb-0">
                <tr><th>{{ __('reports.tithes') }}</th><td class="text-right">{{ number_format($financial['tithes'], 2) }}</td></tr>
                <tr><th>{{ __('reports.offerings') }}</th><td class="text-right">{{ number_format($financial['offerings'], 2) }}</td></tr>
                <tr><th>{{ __('reports.pledge_payments') }}</th><td class="text-right">{{ number_format($financial['pledges'], 2) }}</td></tr>
                <tr><th>{{ __('reports.bereavements') }}</th><td class="text-right">{{ number_format($financial['bereavements'], 2) }}</td></tr>
            </table>
        </div>
    </div>
    <div class="col-lg-6 mb-4">
        <div class="tile">
            <h3 class="tile-title">{{ __('reports.weekly_breakdown') }}</h3>
            <table class="table table-sm table-hover mb-0">
                <thead><tr><th>{{ __('reports.week') }}</th><th class="text-right">{{ __('reports.income') }}</th><th class="text-right">{{ __('reports.expenses') }}</th></tr></thead>
                <tbody>

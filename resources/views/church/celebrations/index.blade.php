...
<div class="app-title">
    <div>
        <h1><i class="fa fa-birthday-cake"></i> Celebrations</h1>
        <p>Year <strong>{{ $selectedYear }}</strong> — {{ $periodLabel }}</p>
    </div>
    <ul class="app-breadcrumb breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('church.dashboard') }}">Dashboard</a></li>
        <li class="breadcrumb-item">Celebrations</li>
    </ul>
</div>
...
<div class="tile mb-3">
    <form method="GET" class="form-inline align-items-end flex-wrap">
        <div class="form-group mr-3 mb-2">
            <label class="small d-block mb-1">Year</label>
        ...
        <div class="form-group mb-2">
            <small class="text-muted d-block">
                Past celebrations in {{ $selectedYear }} are hidden until next year.
                @if(!$isCurrentYear)
                    Viewing a future/past year — all dates in that year are shown.
                @endif
            </small>
        </div>
    </form>
    ...
    <div class="col-md-3">
        <a href="{{ route('church.celebrations.index', ['year' => $selectedYear, 'period' => 'this_month', 'celebration_type' => 'birthday']) }}" class="text-decoration-none">
            <div class="tile text-center p-3 h-100">
                ...
                <small class="text-muted">Birthdays in {{ $monthLabel }}</small>
            </div>
        </a>
    </div>
    <div class="col-md-3">
        <a href="{{ route('church.celebrations.index', ['year' => $selectedYear, 'period' => 'this_month', 'celebration_type' => 'wedding_anniversary']) }}" class="text-decoration-none">
            <div class="tile text-center p-3 h-100">
                ...
                <small class="text-muted">Anniversaries in {{ $monthLabel }}</small>
            </div>
        </a>
    </div>
</div>
...
<div class="tile mb-3">
    <h5 class="mb-2">Quick period — {{ $selectedYear }}</h5>
    <div class="btn-group flex-wrap mb-2" role="group">
        <a href="{{ route('church.celebrations.index', ['year' => $selectedYear, 'period' => 'this_month']) }}"
            ...
            {{ $isCurrentYear ? now()->format('F') : ($monthOptions[$displayMonth] ?? 'Month') }}
        </a>
        @if($isCurrentYear)
        ...
        <a href="{{ route('church.celebrations.index', ['year' => $selectedYear, 'period' => 'all']) }}"
            ...
            Rest of {{ $selectedYear }}
        </a>
    </div>

    <h5 class="mb-2">Browse by month — {{ $selectedYear }}</h5>
    <p class="text-muted small mb-2">Birthdays & anniversaries in each month. Already-celebrated dates this year are hidden.</p>
    ...
    <form method="GET" class="form-row align-items-end">
        ...
        <div class="form-group col-md-4 mb-2">
            <label class="small mb-1">Month in {{ $selectedYear }}</label>
        ...
        <div class="form-group col-md-2 mb-2">
            <button type="submit" class="btn btn-sm btn-success btn-block">View</button>
        </div>
    </form>
</div>
    ...
    <div class="col-md-9">
        <form method="GET" class="form-inline flex-wrap">
            ...
            @if(!empty($filters['month']))
                <input type="hidden" name="month" value="{{ $filters['month'] }}">
            @endif
            <input type="text" name="search" class="form-control mr-2 mb-2" placeholder="Search member or title..."
            ...
            <a href="{{ route('church.celebrations.index', ['year' => $selectedYear]) }}" class="btn btn-light mb-2 ml-1">Reset</a>
        </form>
    </div>
    <div class="col-md-3 text-md-right">
        @can('create', \App\Models\Celebration::class)
            <a href="{{ route('church.celebrations.create') }}" class="btn btn-primary mb-2">
                <i class="fa fa-plus"></i> Add Celebration
            </a>
        @endcan
        <form method="POST" action="{{ route('church.celebrations.sync') }}" class="d-inline">
            @csrf
            <button type="submit" class="btn btn-outline-secondary mb-2" title="Refresh from member profiles">
                    ...
                    <tr class="{{ $daysUntil !== null && $daysUntil <= 7 ? 'table-warning' : '' }}">
                        <td nowrap>
                            ...
                            @if($item->original_date)
                                <br><small class="text-muted">Born/wed: {{ $item->original_date->format('M d') }}</small>
                            @endif
                            @if($item->yearsCount())
                                <br><small class="text-muted">{{ $item->yearsCount() }} {{ $item->celebration_type === \App\Enums\CelebrationType::Birthday ? 'years' : 'years married' }}</small>
                            @endif
                        </td>
                        <td nowrap>
                            @if($daysUntil === 0)
                        ...
                        <td>
                            <a href="{{ route('church.celebrations.show', $item) }}" class="btn btn-sm btn-info" title="View"><i class="fa fa-eye"></i></a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="text-muted text-center py-4">
                            No celebrations found for <strong>{{ $periodLabel }}</strong>.
                            @if($isCurrentYear)
                                Dates that already passed this year will appear again next year.
                            @endif
                        </td>
                    </tr>
                @endforelse
